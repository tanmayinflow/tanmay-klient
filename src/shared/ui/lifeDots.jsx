...
export function LifeDots({ total, lived, t, L }) {
  ...
  useEffect(() => {
    ...
    art.onload = draw;
    art.onerror = draw;
    art.src = "/media/landscape/memento-art-v4.png";
  ...
  return <div ref={wrap} className="tm-life-dots"><canvas ref={canvas} role="img" aria-label={L(
    `${lived} prožitých dní z přibližně ${total}. Každá tečka je jeden den. Tmavší tečky značí prožitý čas. V tečkách vystupuje slabika ÓM, pod ní zdobená lebka a níže slabika HUNG.`,
    `${lived} days lived out of approximately ${total}. Each dot is one day. Darker dots mark lived time. The dots form the OM syllable, an ornate skull below it, then the HUNG syllable.`
